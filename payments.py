...
def deliver_product(chat_id, username, first_name, name_good, amount, sum_amount, payment_method):
    ...
    try:
        ...
        if name_good.startswith('SUB:'):
            ...
            product = subscriptions.get_subscription_by_name(name_good)
            sub_id = product[0] if product else None
        else:
            product = None
        ...
        if product:
            ...
            name_good = name
        elif name_good.startswith('SUB:') or dop.is_subscription_product(name_good):
