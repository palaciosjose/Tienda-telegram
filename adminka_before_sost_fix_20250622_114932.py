    ...
    if chat_id in dop.get_adminlist():
        if message_text == 'Volver al menú principal' or message_text == '/adm':
            ...
            if a == 0: 
                user_markup.row('Añadir nueva posición en el escaparate', 'Eliminar posición')
                user_markup.row('Cambiar descripción de posición', 'Cambiar precio')
                user_markup.row('📝 Descripción adicional', 'Otras configuraciones')
                user_markup.row('Volver al menú principal')
...
Envía el nuevo multiplicador:""", reply_markup=key, parse_mode='Markdown')
            ...
            # ESTADOS DE DESCUENTOS - CORREGIDOS
            ...
            elif sost_num == 8:  # Estado para seleccionar producto para editar descripción adicional
                con = sqlite3.connect(files.main_db)
                cursor = con.cursor()
                a = 0
                ...
                for i in cursor.fetchall():
                    a += 1
            if a == 0:
                bot.send_message(chat_id, '¡La posición seleccionada no se encontrada! Selecciónela haciendo clic en el botón correspondiente.')
            else:
        ...
        elif sost_num == 9:  # Estado para recibir nueva descripción adicional
